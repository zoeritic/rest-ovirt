...
#        print_ok(resp)
        ...
        print_err("Error:: status:%s"%resp['status'])
        print_info(content)
        return None
...
def vm_start(Mipp,vmid,boot='hd',cdrom=''):
    if cdrom=='':
...
def vdsClient_wrap(vmname):
    CMD=r"vdsClient -s 0 list table|grep "+vmname+r" |awk '{print $2,$3}'"
    fp=os.popen(CMD)
    vms_h=fp.readlines()
    n2p={}
    for ln in vms_h:
        pn=ln.split(' ')
        n2p[pn[1].strip()]=pn[0].strip()
    if len(n2p)!=1:
        print_warn(n2p)
    return n2p
...
def kill_vm(vmname,vmpid):

    os.system('kill -9 '+pid)
    if rt!=0:
        print_err("KILL VM "+vmname)
        return False
    else:
        return True
    #fi
